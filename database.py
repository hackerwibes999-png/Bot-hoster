...
# Use /tmp for Render, local directory for development
if os.getenv('RENDER'):
...
def add_bot(bot_id: str, user_id: int, bot_token: str, bot_type: str, main_file: str):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"✅ Bot {bot_id} added to database")
        return True
    except Exception as e:
...
def get_user_bots(user_id: int) -> List[Dict]:
    ...
    try:
        ...
        print(f"❌ Failed to get user bots: {e}")
        return []
...
def delete_bot(bot_id: str):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"✅ Bot {bot_id} deleted from database")
    except Exception as e:
...
def get_user_bot_count(user_id: int) -> int:
    ...
    try:
        ...
        print(f"❌ Failed to get bot count: {e}")
        return 0
...
def get_user_max_bots(user_id: int) -> int:
    ...
    try:
        ...
        print(f"❌ Failed to get max bots: {e}")
        return 5
